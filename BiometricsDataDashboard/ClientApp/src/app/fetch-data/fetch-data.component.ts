...
import { mergedData } from './data/merged-kevin';
...
export class FetchDataComponent implements OnInit, AfterViewInit{
  ...
  public ngOnInit() {
    ...
    var i;
    var startTime = allLogs.length ? allLogs[0]['ts'] : 0;
    for (i = 0; i < allLogs.length; i++) {
      var log = allLogs[i];
      if (log['Pressure (mmHg)']) {
        this.data1.push({ 'x': log['ts'] - startTime, 'y': log['Pressure (mmHg)'] });
      }
      if (log['HeartRate (bpm)']) {
        this.data2.push({ 'x': log['ts'] - startTime, 'y': log['HeartRate (bpm)'] });
      }
    }
  }
